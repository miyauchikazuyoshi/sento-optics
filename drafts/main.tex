...
The framework does not replace band-gap theory but subsumes it: $E_g$
determines \emph{at which energy} the response occurs, while
$\delta \times D_\mathrm{eff}$ determines its \emph{strength and
directionality}. Extensions to liquid, interfacial, and catalytic systems
are proposed as targets for future independent verification.
\end{abstract}
...
different mechanism. This dual origin is summarized schematically in
...
dimensionality of electronic conduction. The band gap $E_g$ is a
\emph{consequence} of $\delta$, not its cause: the hopping integral~$t$
that governs $\delta$ simultaneously determines the bandwidth
$W \sim zt$ and, through the competition with on-site potentials, the
magnitude of~$E_g$.
...
$D_\mathrm{eff}$ is defined as the \textbf{number of reciprocal-space
directions along which the electronic band dispersion $E(\bm{k})$
exhibits significant dispersion near the Fermi level or band edges},
independently of the crystallographic dimensionality. Specifically:

\begin{equation}
  D_\mathrm{eff} = \#\{i : W_i > W_\mathrm{thresh}\},
  \label{eq:Deff}
\end{equation}

\noindent where $W_i$ is the bandwidth along the $i$-th principal
reciprocal-lattice direction within the energy window relevant to
visible-light transitions ($0$--$4$~eV from the band edge), and
$W_\mathrm{thresh}$ is a threshold bandwidth (we adopt
$W_\mathrm{thresh} = 0.5$~eV as a practical cutoff).

This definition is \emph{purely electronic and structural}; it makes no
reference to optical measurements and therefore avoids circularity.
...
\subsection{Tight-binding proof-of-concept simulation}

A minimal tight-binding model was constructed for four systems (diamond,
1D chain as SWCNT proxy, graphene, and C$_{60}$ cluster) to compute band
structures, densities of states, and an IPR-based delocalization proxy.
The purpose is to verify the $\delta$--$E_g$ inverse correlation within
a self-consistent computational framework, not to replace published data.
...
\subsection{Tight-binding proof of concept}
...
model systems (Pearson $r = -0.86$; Fig.~\ref{fig:TB_corr}), confirming
the expected relationship within a self-consistent computational
framework.
...
\subsection{$E_g$ as an intermediate variable within the
  $\delta \times D_\mathrm{eff}$ hierarchy}

The central claim of this work:
\begin{itemize}
  \item The band gap $E_g$ is a consequence of $\delta$ (via the
    hopping integral~$t$), not an independent root variable.
  \item $E_g$ determines \emph{at which energy} the optical response
    occurs; $\delta \times D_\mathrm{eff}$ determines its
    \emph{strength and directionality}.
  \item The conventional one-axis classification
    (metal/semiconductor/insulator by $E_g$) is extended to a two-axis
    classification by $\delta \times D_\mathrm{eff}$.
\end{itemize}
...
\begin{tabular}{ll}
  ...
  Band theory & $E_g$ is one consequence of $\delta$ and $D_\mathrm{eff}$ \\
...
The framework does not replace band-gap theory; it subsumes it.
$E_g$ determines \emph{at which energy} the response occurs, while
$\delta \times D_\mathrm{eff}$ determines its \emph{strength and
directionality}. The extension of $\delta$ to nuclear degrees of
freedom (liquids), interfacial systems (catalysis), and cross-element
materials is proposed as a target for future independent verification.
