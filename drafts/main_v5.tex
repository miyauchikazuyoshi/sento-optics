...
a decision tree based on $E_g$ and $D_\mathrm{eff}$ classifies all
...
shifts the interband onset to the experimental gap $E_g = 5.47$~eV,
and the high-frequency dielectric constant
$\varepsilon_\infty = 5.7$ is imposed to account for
...
glass reference ($n = 1.567$) at the same angle. Standard measurement
...
respectively). For diamond, we apply a scissors correction
($\Delta = 2.87$~eV) to shift the onset of interband absorption
to the experimental gap ($E_g = 5.47$~eV) and include the
experimental high-frequency dielectric constant
$\varepsilon_\infty = 5.7$ to account for electronic transitions
not captured by the 8-band model. The Kramers--Kronig transform
...
\begin{enumerate}
  ...
  \item Diamond's visible-range reflectivity ($R_\mathrm{vis} = 0.17$)
    ...
    $\varepsilon_\infty = 5.7$ as input; a fully \emph{ab initio} prediction
