...
Systems with high $\delta \times D_\mathrm{eff}$ maintain their optical
response under disorder, while low-$\delta \times D_\mathrm{eff}$
...
\emph{Energy windowing and the h-BN case.}
The sum in Eq.~\ref{eq:Deff} runs over \emph{all} occupied-to-unoccupied
interband transitions, without restriction to a particular energy window.
For h-BN, the $\pi$-band dispersion is substantial ($W_\pi \approx
2.4$~eV), but the band gap $E_g = 5.95$~eV places all interband
transitions in the deep UV. The resulting velocity matrix elements
$v_{nm}$ are nonzero, and $T_{\alpha\beta}$ reflects the in-plane
anisotropy. However, none of these transitions contribute to the
\emph{visible-range} optical response, which is the target of our
classification.

We acknowledge that $D_\mathrm{eff}$ and $E_g$ are not fully
independent: a large $E_g$ reduces the density of visible-energy
transitions, indirectly suppressing the optical weight that enters
$T_{\alpha\beta}$. The key point is that $D_\mathrm{eff}$ provides
\emph{additional} information beyond $E_g$: among materials with the
same $E_g \approx 0$ (graphite, metals, SWCNTs), $D_\mathrm{eff}$
correctly resolves the luster degeneracy that $E_g$ alone cannot lift.
The partial correlation with $E_g$ is a feature, not a bug---it
reflects the physical reality that both variables derive from the
same band structure.

\emph{Limitation: frequency-resolved $D_\mathrm{eff}(\omega)$.}
A more rigorous formulation would restrict the sum in
Eq.~\ref{eq:Deff} to transitions within an energy window
$[\omega - \Delta\omega, \omega + \Delta\omega]$, yielding a
frequency-resolved $D_\mathrm{eff}(\omega)$ that tracks which
spatial directions contribute to the optical response at each photon
energy. The present work uses the all-energy definition for
simplicity and because it is sufficient to classify the systems
studied here. However, for materials where qualitatively different
transitions dominate in different spectral ranges (e.g., interband
$d$-to-$s$ vs.\ intraband Drude in transition metals), a
frequency-resolved $D_\mathrm{eff}(\omega)$ would be necessary.
...
Tables~\ref{tab:bandwidth}--\ref{tab:bandgap} were obtained from
...
analysis (Secs.~\ref{sec:results}--\ref{sec:disorder}) relies
...
\begin{equation}
  ...
  \label{eq:corr}
\end{equation}
A value near unity means the spectral shape is preserved
(coherence maintained); $\mathrm{corr} \ll 1$ means spectral
features are destroyed (coherence lost).
...
The main classification (Secs.~\ref{sec:results}--\ref{sec:disorder})
...
delocalization and optical response: $\delta$ and optical absorption
are not merely correlated---they are related by a sum rule.
...
the Wannier spread equals the frequency-weighted integral of optical
absorption, providing a rigorous mathematical foundation for the
$\delta$--optical connection proposed here.
