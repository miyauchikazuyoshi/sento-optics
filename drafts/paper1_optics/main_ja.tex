...
我々は、光沢の本質が界面における光子の位相コヒーレンスの保存であり、
その微視的条件が\emph{構成粒子の自由度}に帰着すると提案する。
金属では非局在化した電子が集団的応答を可能にし、
液体では分子の移動性が界面の自己平滑化を保証する。
いずれの機構も入射光子の位相関係を保存し、鏡面反射をもたらす。

この考えを厳密に検証するために、固体電子系に範囲を限定し、
有効非局在化指数~$\delta$ と有効伝導次元数~$D_\mathrm{eff}$ の
二つの変数を導入する。
5種の炭素同素体（ダイヤモンド、C$_{60}$、SWCNT、グラフェン、グラファイト）
と対照系としての六方晶窒化ホウ素（h-BN）を用い、
$E_g$ と $D_\mathrm{eff}$ に基づく決定木が7物質中6物質（85.7\%）を
正しく分類することを示す。唯一の例外---単層グラフェン---には
第三の変数として層数~$N$ が必要となる。

概念実証として、各系について tight-binding ハミルトニアン $\to$
久保公式 $\to$ $\varepsilon(\omega)$ $\to$ Fresnel 反射率
の完全な計算連鎖を実行する。
次に、Anderson 無秩序シミュレーションを通じてコヒーレンス保存仮説を検証し、
高い $\delta \times D_\mathrm{eff}$ を持つ系が無秩序下でも光学応答を
維持すること（1D鎖で $W = 5$~eV においてスペクトル相関 $>0.94$）を示す一方、
低い $\delta \times D_\mathrm{eff}$ の系は急速にデコヒーレンスする
（C$_{60}$ で相関 $= 0.66$）ことを確認する。
コヒーレンス保存因子に基づく現象論的な\emph{有効グロス}推定値を提案するが、
これがまだ定量的な光沢理論を構成するものではないことを強調する。

本枠組みはバンドギャップ理論を置き換えるものではなく、
それを拡張するものである。$E_g$ は応答が\emph{どのエネルギーで}
生じるかを決定し、$D_\mathrm{eff}$ はその\emph{方向性}を決定する。
粒子の自由度（$\delta$）の概念が $D_\mathrm{eff}$ の導入を動機づけた。
$\delta$ が液体系や界面系における測定可能な変数として独立に検証されることを
将来の目標として提案する。
\end{abstract}
...
従来の物理学は、金属光沢を Drude モデルで、
液体の反射を Fresnel 方程式で別々の現象として説明する。
両者を単一の原理から統一的に導く枠組みは存在しない。

\subsection{粒子の自由度と量子障壁}

我々は、界面における光子のコヒーレンス保存の条件が、
\emph{構成粒子の自由度}に帰着すると提案する。

物質の界面は入射光子に対する量子障壁として作用する。
この障壁の性質が光学応答を決定する：

\begin{itemize}
  \item \textbf{局在した粒子}（$\delta$ が低い）：各散乱体が独立に応答し、
    再放出される位相はランダム化される。
    障壁は非コヒーレントとなる。結果：拡散散乱（透明または白色）。
  \item \textbf{非局在化した粒子}（$\delta$ が高い）：散乱体が集団的かつ
    コヒーレントに応答し、再放出される位相は揃えられる。
    障壁はコヒーレントとなる。結果：鏡面反射（光沢）。
\end{itemize}

金属では、電子の非局在化（自由電子）がこの条件を満たす。
液体では、分子の非局在化（流動性）が界面の自己平滑化を保証し、
異なる機構を通じて同じ条件を満たす。

\subsection{範囲：固体炭素における概念実証}

液体では、電子の自由度（$\delta_\mathrm{elec}$）と
原子核の自由度（$\delta_\mathrm{nuc}$）が同時に変化するため、
両者の効果を分離することが困難である。固体では
$\delta_\mathrm{nuc}$ が凍結されるため、
$\delta_\mathrm{elec}$ の効果を単離できる。

本研究は\textbf{概念実証}である。利用可能な最も制御された環境において、
包括的な仮説を検証する。炭素同素体を選んだ理由は、
...
液体系、界面系、触媒系への拡張は本枠組みの自然な帰結であり、
将来の独立した実験的検証の目標として提案する。
...
$\delta$ は単一の物理量ではなく、複数の代理指標の相互整合性から
その存在が推論される包括的変数である：
...
\begin{itemize}
  \item \textbf{極性/イオン性}（$p$）：$\pi$ 電子の局在化を通じて
    h-BN の白色性に寄与する。
  \item \textbf{励起子効果}：バンド端近傍の光学応答を修正する。
...
全ての数値データは出版された文献および公開データベースから取得した。
第一原理計算は実施していない。
...
\begin{itemize}
  ...
  \item \textbf{界面と触媒}：ヘテロ接合における $\delta$ の不連続性が
    電荷移動速度を支配するかどうか。
    触媒活性のdバンド中心理論との関連。
  \item \textbf{第一原理 $\delta$}：IPR、Wannier スプレッド、
    光学伝導率のDFTによる系統的計算により、$\delta$ を
    定量的で代理指標に依存しない基盤に置く。
...
関連し得る。同一の変数が光学応答と熱力学的界面特性を
異なる物質分類にわたって橋渡しできるかどうかを探ることは、
自然な次のステップである。
