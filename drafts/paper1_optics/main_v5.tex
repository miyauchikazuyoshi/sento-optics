...
\noindent The $\mathrm{corr}^2$ factor is motivated by the
analogy to coherent scattering: the specular intensity of a
disordered interface scales as the square of the coherent
amplitude fraction. If atomic-scale disorder modulates the local
$\varepsilon(\omega)$, the fraction of the reflected field that
adds constructively (specular peak) is proportional to $\mathrm{corr}$,
and the intensity scales as $\mathrm{corr}^2$. We emphasize that
this is a \emph{minimal phenomenological model}, not a rigorous
derivation from surface-scattering theory (e.g.,
Beckmann--Spizzichino); the latter would require explicit surface
roughness statistics (RMS height, correlation length) that are not
modeled here. Nevertheless, Eq.~\ref{eq:eff_gloss} yields a testable
