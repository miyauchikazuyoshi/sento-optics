...
この欠落した鍵を与えることを提案する。
11種の金属ダイマーに対するDFT計算により、
IPRに基づく非局在化指標~$\delta_\mathrm{IPR}$は
境界電子密度と相関することを示した（$r = 0.84$, $p = 0.001$）。
この結果は3種の基底関数系に対して頑健である
（def2-SVP/TZVP/QZVP: $r = 0.72$--$0.84$）。
5種の金属（Na, K, Al, Cu, Zn）に対する周期的スラブ計算と
価電子分解により、sp金属はほぼ均一な格子間電子密度を示し
（Na: 1.09, K: 1.13, Al: 0.96）、一方$d$金属は
強い枯渇を示すことが明らかになった（Cu: 0.33, Zn: 0.28）
---spと$d$の平均値の比は3.5倍に達する。
アブレーション研究により、全電子密度はアルカリ金属に対して
この傾向を隠蔽すること、価電子分解によって
5種すべての金属で正しいsp $>$ $d$の序列が回復されることが示された。
...
最もよく例示される。
アルミニウム（$r_s = 2.07$, $\gamma = 1140$~mN/m）と
...
\begin{enumerate}
  \item \textbf{IPRに基づく非局在化}：
    ...
    $p_\mu^{(n)} = |c_{n\mu}|^2 S_{\mu\mu}$を計算した。
    軌道ごとのIPRは
    $\mathrm{IPR}_n = \sum_\mu (p_\mu^{(n)})^2 / (\sum_\mu p_\mu^{(n)})^2$
    であり、$\delta_\mathrm{IPR}$は
    式~\eqref{eq:delta_ipr}から求められる。
...
IPRに基づく非局在化を
de~Boer \textit{et al.}~\cite{deBoer1988}が表にまとめた
$n_\mathrm{ws}$の値と、11種のダイマー元素
（Li, Na, K, Be, Mg, Ca, Al, Cu, Zn, Si, Ga）について比較した。
表~\ref{tab:delta_nws}に$\delta_\mathrm{IPR}$を
...
$\delta_\mathrm{IPR}$と$n_\mathrm{ws}$のPearson相関は
中程度（$r = 0.53$, $p = 0.09$）であり、
$\delta_\mathrm{IPR}$と$\gamma$の相関は弱い
（$r = 0.08$、有意でない）。
これは$\delta_\mathrm{IPR}$が$\gamma$の
直接的な予測因子では\emph{ない}ことを確認しており、
説明的な記述子であるという我々の枠組みと整合する。
中程度の$\delta$--$n_\mathrm{ws}$相関は
実質的だが完全ではない関係を反映している：
$\delta_\mathrm{IPR}$は非局在化した電子がセル境界を
占める\emph{傾向}を捉えるが、
実際の境界密度は原子半径、電子数、軌道角運動量にも
依存する---これらはMiedemaの$n_\mathrm{ws}$が
経験的較正を通じて暗黙的に組み込んでいる因子である。
...
\subsection{$\delta_\mathrm{IPR}$は境界密度と相関する}
...
遷移金属にまたがる11元素の等核ダイマーに対して、
$\delta_\mathrm{IPR}$と境界密度比の相関は$r = 0.84$
（$n = 11$, $p = 0.001$）である。
...
\begin{figure}[htb]
  ...
  \caption{$\delta_\mathrm{IPR} \to n_\mathrm{ws}$の
    ...
    （$N = 2$, 11元素）：Pearson $r = 0.84$, $p = 0.001$。
    このパネルは、異なる元素の非局在化がその境界密度を
    決定するという中心的主張を検証する。
    (f)~提案する因果関係の連鎖$\delta \to n_\mathrm{ws} \to \gamma$。
    \emph{注：}本文中で報告する元素間相関（$r = 0.84$）は
    パネル(b)ではなくパネル(e)を指す。}
  \label{fig:test7}
\end{figure}


\subsection{5金属スラブ計算：価電子の非局在化}

提案するメカニズムの最も直接的な検証は、
5種の金属にわたる周期的DFTスラブ計算から得られる。
...
\begin{enumerate}
  \item \textbf{元素のカバレッジ。}
    5種の金属（Na, K, Al, Cu, Zn）はsp/$d$の
    トレンドを確認するが、部分的に充填された$d$バンドを持つ
    遷移金属（Fe, Ni, Ti）や$f$電子金属（Ce, La）への
    拡張が一般性の確立に必要である。
    ダイマー計算は11元素を対象とするが、
    いくつかの技術的に重要な金属（Ag, Au, Pt）を除外している。

  \item \textbf{$\delta_\mathrm{IPR}$は説明的であり予測的ではない。}
    ダイマーの$\delta_\mathrm{IPR}$と
    Miedemaの$n_\mathrm{ws}$の直接相関は中程度（$r = 0.53$）であり、
    $\gamma$との相関は有意でない（$r = 0.08$）。
