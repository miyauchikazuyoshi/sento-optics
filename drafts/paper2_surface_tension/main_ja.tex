...
弱い（$r = 0.20$, $p = 0.47$; $n = 15$）であり、
$\delta_\mathrm{IPR}$と$\gamma$の相関は有意でない（$r = -0.21$）。
価電子のみの$\delta_\mathrm{IPR}^\mathrm{val}$は
$n_\mathrm{ws}$相関を改善する（$r = 0.33$, $p = 0.23$）が、
...
\begin{enumerate}
  \item \textbf{元素のカバレッジ。}
    ダイマー計算は、部分的に充填された$d$バンドを持つ遷移金属
    （Fe, Ni, Ti）および$4d$金属（Ag）を含む15元素を対象とする。
    重い$5d$金属（Au, Pt, W）は相対論的擬ポテンシャルまたは
    より大きな基底関数を必要とし、$f$電子金属（Ce, La）とともに
    将来課題とする。
    スラブ計算（5種: Na, K, Al, Cu, Zn）はsp/$d$トレンドを
    確認するが、新たに追加した遷移金属への拡張が有益であろう。

  \item \textbf{$\delta_\mathrm{IPR}$は説明的であり予測的ではない。}
    全電子ダイマー$\delta_\mathrm{IPR}$と
    Miedemaの$n_\mathrm{ws}$の直接相関は弱い
    （$r = 0.20$, $p = 0.47$; $n = 15$）であり、
    $\gamma$との相関は有意でない（$r = -0.21$）。
    価電子のみの$\delta_\mathrm{IPR}^\mathrm{val}$は
    $n_\mathrm{ws}$相関を改善する（$r = 0.33$）が、
    ダイマーレベルでは強い予測因子ではない。
