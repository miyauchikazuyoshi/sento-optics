...
$\delta_\mathrm{IPR}$とMiedemaの$n_\mathrm{ws}^{1/3}$のPearson相関は
ほぼゼロである（全電子$r = -0.03$, $p = 0.90$; $n = 15$、
価電子のみ$r = 0.10$, $p = 0.73$）。
$\gamma$との相関も有意でない（$r = -0.18$）。
$\delta_\mathrm{IPR}$は$n_\mathrm{ws}$や$\gamma$の
直接的な予測因子ではない。ただしSec.~\ref{sec:slab}で示すように、
DFTスラブの絶対中間点密度$n_\mathrm{mid}$は
$n_\mathrm{ws}^{1/3}$と強く相関する（$r = 0.79$, $p = 0.0007$）。
...
相関しない（$r = 0.10$; こちらは直接的な非局在化と間接的な
...
\begin{enumerate}
  ...
  \item \textbf{$\delta_\mathrm{IPR}$は説明的であり予測的ではない。}
    $\delta_\mathrm{IPR}$とMiedemaの$n_\mathrm{ws}^{1/3}$の
    直接相関はほぼゼロ（全電子$r = -0.03$、価電子のみ$r = 0.10$）。
    $\gamma$との相関も有意でない（$r = -0.18$）。
    ダイマーレベルの$\delta$は$n_\mathrm{ws}$の直接予測因子ではない。
