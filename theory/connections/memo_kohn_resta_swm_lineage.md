...
*2026-07-05 数値実証（simulation/resta_nuclear/ 実験E）*: TB で初の明示的計算。
1D で RS Fig.2 を定性再現（絶縁体収束・金属発散）。理想化グラファイト
（面内金属 × 層間 t_z=0）で **λ_z² = 0（厳密）かつ λ_x² 発散 = D_eff=2 を実証**。
t_z > 0 の弱3D金属では単一 twist の有限系にシェル効果が残り、滑らかな
λ_z² には twist 平均（SWM Eq. 32 の境界条件平均そのもの）が必要 — 将来課題。
